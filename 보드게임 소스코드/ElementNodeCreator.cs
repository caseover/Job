...
namespace MyDice.Board
{
    public class ElementNodeCreator : MonoBehaviour
    {
        ...
        private void updatePlayerGame_CPU()
        {
            ...
            {
                int[] new_DiceValues = new int[] {2,0};
                // int[] new_DiceValues = diceManager.getDicesValues();
                float summed = 0;
                if (captured)
                {
                    for(int i = 0; i < new_DiceValues.Count(); i++)
                    {
                        int a = new_DiceValues[i];
                        summed += a;
                    }
                    int rounded_DiceValues = (int)Mathf.Round(summed/2);
                    new_DiceValues = new int[]{rounded_DiceValues,0}; 
                }
